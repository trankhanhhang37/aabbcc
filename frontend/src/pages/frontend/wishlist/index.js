...
export default function Wishlist() {
    ...
    useEffect(() => {
        userInfo && (
            !wishList && dispatch(getWishList({ userId: userInfo._id }))
        )
        !allProducts && dispatch(onAllProduct({ limit: 100, page: 1 }))

    }, [wishList, userInfo, allProducts]);
                                ...
                                <div class="content-body">
                                    <div class="col-lg-12">
                                        ...
                                        <div class="row justify-content-center mb-3">
                                            {wishList ? wishList.wish_list_products.map((product_in_wish_list) => {
                                                allProducts && allProducts.map((product, index2) => {
                                                    if (product_in_wish_list.toString() === product._id.toString()) {
                                                        console.log("de vao", product)
                                                        return (<ProductListItem key={index2} product={product} />)
                                                    }
                                                })
                                            }):<></>}
